...
export default function GuestGallery({
  ...
  const [cursor, setCursor] = useState<string | null>(
    initialMedia.length > 0
      ? new Date(initialMedia[initialMedia.length - 1].createdAt).toISOString()
      : null
  )
  ...
  async function loadMore() {
    ...
    try {
      ...
      setMedia(prev => {
        ...
        const newItems = data.media.filter((m: MediaItem) => !existingIds.has(m.id))
  ...
  function playVideoFullscreen(mediaItem: MediaItem) {
    ...
    video.src = `/api/photo/${mediaItem.filename}`
    video.controls = true
    video.autoplay = true
  ...
  function addFiles(files: FileList | File[]) {
    ...
    for (const file of Array.from(files)) {
      ...
      newPending.push({ file, preview: URL.createObjectURL(file), caption: '', status: 'idle', progress: 0 })
    }
    ...
    try {
      const interval = setInterval(() => {
        setPending(prev => prev.map((p, i) =>
          i === index && p.progress < 85 ? { ...p, progress: p.progress + 15 } : p
        ))
      }, 200)
  ...
  async function handleUploadAll() {
    ...
    if (uploaded.length > 0) {
      setMedia(prev => {
        const existingIds = new Set(prev.map(m => m.id))
        const newItems = uploaded.reverse().filter(m => !existingIds.has(m.id))
    ...
    <div className="px-4 sm:px-8 pb-16 max-w-6xl mx-auto">
      ...
      {showUploadModal && (
        ...
        >
          <div
            ...
            style={{ backgroundColor: 'var(--ivory)', height: '92dvh' }}
          ...
          >
            ...
            <div style={{ flex: '1 1 0', minHeight: 0, overflowY: 'scroll', WebkitOverflowScrolling: 'touch', padding: '0 1.5rem 1rem' }}>
              ...
              >
                ...
                <input ref={fileInputRef} type="file" accept="image/*,video/*" multiple className="hidden" onChange={onInputChange} />
              </div>
              ...
              {pending.length > 0 && (
                <div className="flex flex-col gap-3">
                  {pending.map((item, i) => (
                    <div key={i} className="flex gap-3 items-start rounded-xl p-3" style={{ backgroundColor: 'white', border: '1px solid rgba(28,28,28,0.06)' }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-sans text-xs truncate mb-1.5" style={{ color: 'var(--muted)' }}>{item.file.name}</p>
                        <input
                          type="text"
                          ...
                          style={{ backgroundColor: 'rgba(28,28,28,0.04)', border: '1px solid rgba(28,28,28,0.08)' }}
                        />
                        {item.status === 'uploading' && (
                          <div className="mt-1.5 rounded-full overflow-hidden h-0.5" style={{ backgroundColor: 'rgba(28,28,28,0.1)' }}>
                            <div className="h-full rounded-full transition-all duration-300" style={{ width: `${item.progress}%`, backgroundColor: 'var(--rose)' }} />
                          </div>
                        )}
                      </div>
                      {item.status === 'idle' && (
                        <button onClick={() => removeFile(i)} className="shrink-0 w-6 h-6 flex items-center justify-center rounded-full text-xs" style={{ color: 'var(--muted)', backgroundColor: 'rgba(28,28,28,0.06)' }}>✕</button>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="px-6 pt-4 flex gap-3 shrink-0" style={{ borderTop: '1px solid rgba(28,28,28,0.08)', paddingBottom: 'calc(1.5rem + env(safe-area-inset-bottom))' }}>
              <button onClick={closeModal} disabled={uploading} className="flex-1 py-2.5 rounded-xl text-sm font-sans font-medium transition" style={{ border: '1px solid rgba(28,28,28,0.15)', color: 'var(--muted)' }}>
                Cancel
              </button>
              <button
                ...
                style={{
                  backgroundColor: pending.length > 0 && !uploading && !allDone ? 'var(--charcoal)' : 'rgba(28,28,28,0.2)',
                  color: 'var(--ivory)',
                }}
              >
      ...
      {lightboxItem && (
        ...
        >
            ...
            {lightboxItem.mediaType === 'video' ? (
              <video
                ...
                onCanPlay={() => {
                  ...
                  if (!video || window.innerWidth >= 640) return
                  if ((video as any).webkitEnterFullscreen) {
                    (video as any).webkitEnterFullscreen()
                  } else if (video.requestFullscreen) {
                    video.requestFullscreen()
          ...
          <button className="absolute right-4 sm:right-8 text-white/50 hover:text-white text-3xl z-10 transition" onClick={e => { e.stopPropagation(); lightboxNext() }}>›</button>
          <button className="absolute top-4 right-4 text-white/50 hover:text-white text-xl transition" onClick={() => setLightboxItem(null)}>✕</button>
        </div>
      )}
    </div>
  )
